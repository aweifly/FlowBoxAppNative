package com.iknet.bluetoothmeasure.util;
...
//		}
	...
	public static String byte2hex(byte[] b) { // 一个字节的数，
		...
		for (int n = 0; n < b.length; n++) {
			...
			tmp = (Integer.toHexString(byte2Int(b[n])));
			if (tmp.length() == 1) {
				hs = hs + "0" + tmp;
			} else {
				hs = hs + tmp;
	 ...
	 * @return
	 */
	public static int ascii2Int(byte[] b){
		int result = 0;
//		int[] ints = new int[b.length];
		StringBuffer sb = new StringBuffer();
		for(int i=0; i<b.length; i++){
//			ints[i] = byte2Int(b[i]);
			sb.append((char) byte2Int(b[i]));
		}
...
//		result = Float.parseFloat(sb.toString().trim());
	...
	public static String bytes2hex(byte[] b) { // 一个字节的数，
		...
		for (int n = 0; n < b.length; n++) {
			...
			tmp = (java.lang.Integer.toHexString(byte2Int(b[n])));
			if (tmp.length() == 1) {
				hs = hs + "0" + tmp;
			} else {
				hs = hs + tmp;
